...
public class PlayerController_Changed : MonoBehaviour {
	...
	void Update () {
        if (!GlobalVariables.gameOver && !GlobalVariables.win) {
            ...
            if (Input.GetKeyDown(KeyCode.Z) || Input.GetKey(KeyCode.Z)) {
				if(!shooting){
					shooting = true;					
					Bullet_Changed bullet = Instantiate(bulletPrefab, new Vector3(playerRT.position.x, playerRT.position.y + playerRT.sizeDelta.x / 2), Quaternion.Euler(0, 0, 0), gameObject.transform.parent).GetComponent<Bullet_Changed>();
